...
export default function KitchenPage() {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      const initialPlans = textData.plans.map(
        (plan: Omit<Plan, "imageLoading" | "image">) => ({
          ...plan,
          image: undefined,
          imageLoading: true,
        })
      ...
      initialPlans.forEach(async (plan: Plan, index: number) => {
        try {
          ...
          // base64データを受け取ってdata URLとして使用
          const base64Data = await imageResponse.text();
          const imageUrl = `data:image/png;base64,${base64Data}`;

          console.log(
            `Generated image for ${plan.title}:`,
          ...
          setPlans((currentPlans) =>
            currentPlans.map((p, i) =>
              i === index ? { ...p, image: imageUrl, imageLoading: false } : p
            )
          );
        } catch (e) {
